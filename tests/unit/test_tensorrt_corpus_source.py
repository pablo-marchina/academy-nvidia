...
def test_tensor_rt_source_is_governed_fresh_and_hash_consistent() -> None:
    ...
    sources = yaml.safe_load((CORPUS_ROOT / "sources.yaml").read_text(encoding="utf-8"))["sources"]
    allowlist = yaml.safe_load((CORPUS_ROOT / "source_allowlist.yaml").read_text(encoding="utf-8"))["sources"]
