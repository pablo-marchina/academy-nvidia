...
Build a reproducible, versioned, AI-oriented workspace that prioritizes traceability, evidence quality, structured outputs, and continuous evaluation. Productization is guided by the consolidated final backlog in [docs/54_final_product_backlog.md](docs/54_final_product_backlog.md) and the benchmark-first final roadmap in [final_final_benchmark_first_roadmap_all_changes.md](final_final_benchmark_first_roadmap_all_changes.md).
...
Final proof artifacts are generated under `final_case_evidence/` with `make evidence-pack`. The one-command final gate is `make prove-final-product`, which prints `FINAL_PRODUCT_STATUS=PASS_OR_FAIL`.

External reviewers should use [docs/final_external_reviewer_mode.md](docs/final_external_reviewer_mode.md) for clean-environment reproduction. The final delivery ZIP is generated by `make package-final-release` and validated by `make check-final-release-zip`; it is an allowlisted source release, not a ZIP of the working directory.
...
5. **Analysis Run** — evidence-first bundle with confidence, coverage, unsupported claims, recommendations, missing evidence, RAG support, lineage, plus full run detail
...
The full product proof requires real PostgreSQL, Qdrant, RAG corpus ingestion,
and product-like acceptance. Use the doctor first when the result is
`BLOCKED_BY_ENVIRONMENT`:

```bash
make local-proof-doctor
make local-services-up
make prove-final-product
make full-proof-pass-attempt
```

On Windows, Docker Desktop must be running and the current user must be able to
read `%USERPROFILE%\.docker\config.json` and access
`npipe:////./pipe/docker_engine`. If Codex cannot access Docker but services are
already running, set `PRODUCT_DB_URL`, `QDRANT_URL`, and `QDRANT_COLLECTION`;
the full proof will use those external local services without deleting volumes,
databases, or collections. Use `--require-docker-compose` only when the proof
must prove that Docker Compose itself can start the services.

Official local routes:

- Docker Compose route: run `make local-services-up`, then `make prove-final-product`.
- External services route: start PostgreSQL and Qdrant outside Codex, set
  `PRODUCT_DB_URL`, `QDRANT_URL`, and `QDRANT_COLLECTION`, then run
  `make full-proof-pass-attempt`.
...
- [ ] Evidence bundle reviewed (`GET /analysis-runs/{id}/evidence-bundle`)
