...
TEST = Path("tests/unit/test_rag_ingestion_release_calibration.py")
MARKER = "RAG ingestion release calibration (2026-08-02)"

FUNCTION = '''def _ingestion_corpus_decisions() -> list[DecisionCalibrationRecord]:
...
TEST_CONTENT = '''from src.quality.decision_calibration_registry import (
    CalibrationStatus,
...
def main() -> int:
    ...
    if MARKER not in content:
        ...
        content = content[:start] + FUNCTION + content[end + 1 :]
        REGISTRY.write_text(content, encoding="utf-8")
        changed = True
    if not TEST.exists() or TEST.read_text(encoding="utf-8") != TEST_CONTENT:
        TEST.write_text(TEST_CONTENT, encoding="utf-8")
        changed = True
    print("changed" if changed else "already_calibrated")
    return 0
